...
orchdat3 |> ggplot2::ggplot(mapping = aes(x = factor(TransectPoint_ID), y = CountSum, group = year)) +
...
# y=b0 + b1x1 + b2*(1^2) # expect b2 to be negative
...
# yes this is a better model, the sum of squares is significantly lower than the previous model
...
anova(m4)
anova(m4,m3)
# the interaction is not significant , so we can remove it from the model 
  
#add the linear model to the plot to it 
# calculate the predicted value of m2 for every observation, add to the dataset as a variable as pred2
# add the new predicted line to the previous plot p2, store as object p3 and show it

  
#### m3 is the preferred model in this case (outcome of model selection)
# we tried to find the best model 

## dinsdag 17 september 2024 

# explore the consequences of a log transformation of y values
x<-c(0,1,2,3,4,5)
y<-c(0,0,100,5000,13900,100000)
dat<-data.frame(x,y)
dat

# ecological response for instance 
...
anova(m5, test = "Chisq")
# elevation has a significant effect on the abundance of Orchestia
