...
base::mean(my_numbers) # from the package base we use the function mean 

```

the sum of 5 and 10 is 15

make a plot with a figure label and a figure caption using the different
prompts
...
In the default settings of R Studio, the output of @fig-histogram
(refers to the figure above) above is produced "inline", so in the the
...
It contains information, using fixed field names, o n the title, author
